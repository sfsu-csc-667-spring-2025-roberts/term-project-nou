...
function startNewGame() {
  ...
  if (topCard.value === 'skip') {
    ...
    gameState.players[gameState.currentPlayer].hand.push(...gameState.deck.splice(0, 2));
    switchPlayer();
  }
}
...
function switchPlayer() {
  gameState.currentPlayer = (gameState.currentPlayer + gameState.direction + 2) % 2;
...
function playCard(playerId, cardIndex) {
  ...
  if (player.hand.length === 1) {
    return { 
      success: true, 
      message: "UNO!", 
      nextPlayer: gameState.players[(playerIndex + gameState.direction + 2) % 2].id 
    };
  }
